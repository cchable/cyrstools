...
Create Table "T_EPHEMERIDES"  (
	...
	"IDX_ANNEESCOLAIRE" Bigint NOT NULL,
	"INTITULEEPHEMERIDE" Varchar(30) NOT NULL,
...
Alter Table "T_CHAUFEURS" add Foreign Key ("IDX_VEHICULE") references "T_VEHICULE" ("IDX_VEHICULE") on update no action on delete no action ;
Alter Table "T_TRANSPORTS" add Foreign Key ("IDX_VEHICULE") references "T_VEHICULE" ("IDX_VEHICULE") on update no action on delete no action ;
...
^
SET TERM ; ^
/* "Autoinc" functionality for T_VEHICULE_IDX_VEHICULE*/ 
CREATE SEQUENCE "SEQ_T_VEHICULE_IDX_VEHICULE";
SET TERM ^ ;
CREATE TRIGGER "TR_BI_T_VEHICULE_IDX_VEHICULE" FOR "T_VEHICULE"
ACTIVE BEFORE INSERT POSITION 0
AS
BEGIN
  IF (NEW."IDX_VEHICULE" IS NULL) THEN NEW."IDX_VEHICULE" = GEN_ID("SEQ_T_VEHICULE_IDX_VEHICULE",1); 
END
^
CREATE PROCEDURE "P_T_VEHICULE_IDX_VEHICULE"
RETURNS (
    "AUTOINC" INTEGER)
AS
BEGIN  
	"AUTOINC" = GEN_ID("SEQ_T_VEHICULE_IDX_VEHICULE",1);
   SUSPEND;
END
^
SET TERM ; ^
