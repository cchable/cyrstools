<?php
/**
 * @package   : module/PlanningBus/src/Model/TrajetFullTable.php: 
 *
 * @purpose   :
 * 
 * 
...
class TrajetFullTable
{
	...
	public function fetchAllPaginator($pageNumber = 1, $count = null, $search = '')
  {
	...
	//
  public function getTrajet($idTrajetFull)
  {
  ...
  public function findOneById(int $id)
  {
    ...
    $ephemerideFull = $this->findOneBy(['IDX_TRAJET' => (int) $id]);
    return $ephemerideFull;
  }
  
  //
  public function findOneByRecord(Trajet $ephemeride)
  {
    
    $ephemerideFull = $this->findOneBy([
      'IDX_ETAPEDEBUT'   => $record->getIdEtapeDebut(),
      'IDX_ETAPEARRIVEE' => $record->getIdEtapeArrivee(),
      'NOMTRAJET'        => $record->getNom(),
      'TEMPSTRAJET'      => $record->getTemps(),
      'KMTRAJET'         => $record->getKm(),
    ]);
    return $ephemerideFull;
  }
}